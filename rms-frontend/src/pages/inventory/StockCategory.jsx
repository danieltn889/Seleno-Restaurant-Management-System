...
export default function StockCategory() {
  const [categories, setCategories] = useState([
    {
      ...
      stockcat_date_created: "2026-01-01",
    },
    {
      stockcat_id: 2,
      stockcat_name: "Beverages",
      stockcat_status: "inactive",
      stockcat_date_created: "2026-01-02",
    },
  ]);
  ...
  const handleAdd = () => {
    const newCategory = {
      stockcat_id: Date.now(),
      stockcat_name: form.stockcat_name,
      stockcat_status: form.stockcat_status,
      stockcat_date_created: new Date().toISOString().split("T")[0],
    };
    ...
    Swal.fire({
      ...
      text: `${newCategory.stockcat_name} has been added successfully.`,
      timer: 2000,
  ...
  /* ================= DELETE ================= */
  const handleDelete = () => {
    const deletedCat = categories.find(c => c.stockcat_id === selectedId);
    setCategories(categories.filter(c => c.stockcat_id !== selectedId));
    setShowDelete(false);

    Swal.fire({
      icon: "success",
      title: "Category deleted!",
      text: `${deletedCat.stockcat_name} has been deleted.`,
      timer: 2000,
      showConfirmButton: false,
    });
  };
  ...
  const handleEdit = () => {
    setCategories(categories.map(c =>
      c.stockcat_id === selectedId ? { ...c, ...form } : c
    ));
    setShowEdit(false);
